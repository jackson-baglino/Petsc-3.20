...
#include <petsc/private/snesimpl.h>  //AM
...
static PetscErrorCode TSStep_Alpha(TS ts)
{
  ...
  PetscInt it0 = 0, NR = 0, rep_ts=0, snes_it0 = 0;  //AM
  PetscReal dt_init; //AM
  ...
  while (!ts->reason && th->status != TS_STEP_COMPLETE) {
    ...
    next_time_step = ts->time_step; //AM
    dt_init = ts->time_step; //AM
    snes_it0 = ts->snes_its;    //AM
    it0 = 0;        //AM
    ...
    if (ts->steprestart) {
      ...
      if (!stageok) {
        if(ts->adap) {  //AM
          ts->time_step = next_time_step/ts->factor; //AM
          if(ts->time_step<ts->dtmin) { //AM
            ts->time_step = next_time_step; //AM
            PetscPrintf(PETSC_COMM_WORLD,"Min dt! can not ");  //AM
          }  //AM
          PetscPrintf(PETSC_COMM_WORLD,"Reduce time step: %.3e -> %.3e (rtol*=10 next TS-iteration)\n",next_time_step,ts->time_step); //AM
        } //AM
        goto reject_step;
      }
      NR = 1 + (ts->snes_its - snes_it0)/3; //AM
    }
    ...
    if (!stageok) {
      if(ts->adap) {                //AM
        ...
        PetscPrintf(PETSC_COMM_WORLD,"Reduce time step: %.3e --> %.3e (rtol*=10 next time iteration)\n",next_time_step,ts->time_step); //AM
      } 
      goto reject_step;
    }

    if(it0==0) NR = ts->snes_its - snes_it0;        //AM
    ...
    if(NR >= ts->NRmax && ts->adap) {           //AM
      ts->ptime += ts->time_step;                     //AM
      ts->time_step = next_time_step/ts->factor;      //AM
      if(ts->time_step<ts->dtmin) {  //AM
        ts->time_step = next_time_step; //AM
        PetscPrintf(PETSC_COMM_WORLD,"Min dt! can not "); //AM
...
//-------------------------------------- max dt dynamic (changes with time)
    if(ts->maxdt_dyn==1){  //AM
      if(ts->time_step > ts->dtmax && ts->adap ) {   //AM
        ts->time_step = next_time_step/ts->factor;   //AM
        PetscPrintf(PETSC_COMM_WORLD,"Max dt! ");  //AM
        PetscPrintf(PETSC_COMM_WORLD,"Decrease time step: %.3e -> %.3e \n",next_time_step,ts->time_step);  //AM
        if(ts->time_step < dt_init) ts->snes->prev_dt_red = 1; else ts->snes->prev_dt_red = 0;  //AM
        break;  //AM
      }  //AM
    }  //AM
//--------------------------------------

    if(NR <= ts->NRmin && ts->adap && rep_ts==0) {  //AM
      ts->time_step = next_time_step*ts->factor;  //AM
      if(ts->time_step>ts->dtmax) {  //AM
        ts->time_step = next_time_step; //AM
        PetscPrintf(PETSC_COMM_WORLD,"Max dt! can not ");  //AM
      }  //AM
      PetscPrintf(PETSC_COMM_WORLD,"Increase time step: %.3e -> %.3e \n",next_time_step,ts->time_step);  //AM
    }      //AM
    if(ts->time_step < dt_init) ts->snes->prev_dt_red = 1; else ts->snes->prev_dt_red = 0;  //AM
